...
def multicreator_glue_everything_and_upload(twitch_clip_page_link="https://something???", video_title="thitchvideo", description="",creatorusername="", titleorig=""):
    func_link = str(twitch_clip_page_link)
...
#     print(make_thumbnail("pokimane", "Out Of Context Pokimema", "14"))
######################################################
...
if __name__ == '__main__':
    ...
    for creator, creatordata in my_DB.items():
        ...
        if creatordata["last_day_update"] < day:
            ...
            title2=(creatordata["capsname"] + " best moments " + day)
            title=title2.replace(" ","\\ ")
            description=("top twitch clips daily " + creatordata["description"] + generic_description)
            description=description.replace(" ","\\ ")
            print(clip+"\n"+title+"\n"+description+"\n")

            uploadresult=multicreator_glue_everything_and_upload(clip,title,description, creatordata["gamename"], titleIMG)
